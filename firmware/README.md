...
战斗测试页使用同一协议：光照 `0x40 hi lo` 会实时更新日光束和光合作用的显示数值；震动 `0x09` 等同电脑键盘 `Z`，用于触发选中的重磅冲撞。电脑端启动命令为 `python battle_demo.py --port COM3` 或 `run_battles.bat COM3`。
