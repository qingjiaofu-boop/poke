...
编辑器还支持三层区域复制：点击左侧的“框选工具 [M]”（或按 `M`，也可按住 `Shift` 后用左键拖动）进入框选模式；按住左键在地图上拖出黄色矩形，松开后按 `Ctrl+C` 复制。再按 `Ctrl+V`，移动鼠标到目标格并点击即可粘贴。粘贴模式会保持开启，可连续点击以重复放置同一棵树或建筑；按 `1/2/3` 返回普通编辑。地面、当前层、上层和当前层碰撞标记会一起复制。
