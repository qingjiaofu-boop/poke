...
快捷键：`1/2/3` 切换三层，`4` 切换游戏最终画面预览，`C` 切换第二层碰撞标记模式（如果当前不是第二层会自动切换），`P` 设置出生点，左键绘制、右键擦除，`←/→` 翻图块页，`G` 输入页码后直接跳转图块页，`S` 保存，`O` 输入地图名打开已有地图。也可以点击左侧的“跳转图块页 [G]”“保存地图 [S]”或“打开地图 [O]”按钮。保存后会更新 `assets/maps/*_lower.png`、`*_current.png`、`*_upper.png` 和 `outdoor_maps.json`，游戏下次启动自动读取。

切换图层后，当前层会以彩色描边高亮，另外两层变暗：蓝色为地面层、橙色为当前层、紫色为上层。红框是第二层的碰撞标记，只保存为 `blocked` 坐标，不会改变素材 PNG；游戏运行时只有这些 `blocked` 坐标会阻挡角色，地面层和上层本身没有碰撞。按 `4` 后三层会按游戏实际顺序正常合成，并隐藏编辑辅助框，便于检查最终效果；按 `1/2/3` 返回编辑。
