...
按 `W` 或点击“箱庭拼接 [W]”可以进入箱庭工作区。左侧会列出 `outdoor_maps.json` 中的所有地图：选中地图后，点击右侧空白处放置；点击已有地图块会选中它，使用方向键逐格移动，`Shift+方向键` 每次移动 5 格，`Delete` 删除选中地图，右键也可删除。画布网格用于对准格子，黄色边框表示当前选中的地图块，青色线段表示两张地图边缘已经准确相接，便于对齐道路和洞口。按 `4` 可隐藏地图名称查看最终拼接效果，按 `S` 会保存 `assets/maps/world_layout.json`，生成 `world_lower.png`、`world_current.png`、`world_upper.png` 三张整体预览图，并导出 `world_connections.json` 和可直接阅读的 `world_connections.txt`：其中记录每张地图的编号、坐标、尺寸、四条边，以及它和另一张地图的哪条边相接、世界坐标范围和双方局部坐标范围。按 `Esc` 返回单地图编辑。
