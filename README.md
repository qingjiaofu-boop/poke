...
按 `W` 或点击“箱庭拼接 [W]”可以进入箱庭工作区。左侧会列出 `outdoor_maps.json` 中的所有地图，点击地图名称后在右侧大画布左键放置，可重复放置不同地图；右键点击地图块范围即可删除最上层的一块。按 `4` 可隐藏地图名称查看最终拼接效果，按 `S` 会保存 `assets/maps/world_layout.json`，并生成 `world_lower.png`、`world_current.png`、`world_upper.png` 三张整体预览图；按 `Esc` 返回单地图编辑。
